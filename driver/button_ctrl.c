...
MODULE_DESCRIPTION("button control for snake game");
MODULE_VERSION("0.1");

static unsigned int gpioButton[4] = {44, 26, 46, 65};
//static unsigned int gpioButton[1] = {44};
/* up down left right */
...
static int dev_open(struct inode *inodep, struct file *filep)
{
	...
	pid_t user_pid;
	t = pid_task(get_task_pid(current, PIDTYPE_PID), PIDTYPE_PID);
	user_pid = pid_nr(get_task_pid(current, PIDTYPE_PID));
