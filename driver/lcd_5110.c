...
void LCD_GotoXY(unsigned char x, unsigned char y)
{
	...
	if (y < 0x08 ) {
		LCD_Write(LCD_COMMAND, LCD_SETYADDR);
		LCD_Write(LCD_COMMAND, LCD_SETXADDR | x);
	}

	if (y > 0x07 && y < 0x10 ) {
		LCD_Write(LCD_COMMAND, setYAddressBank2);
		LCD_Write(LCD_COMMAND, LCD_SETXADDR | x);
	}

	if (y > 0x0F && y < 0x18 ) {
		LCD_Write(LCD_COMMAND, setYAddressBank3);
		LCD_Write(LCD_COMMAND, LCD_SETXADDR | x);
	}

	if (y > 0x17 && y < 0x20 ) {
		LCD_Write(LCD_COMMAND, setYAddressBank4);
		LCD_Write(LCD_COMMAND, LCD_SETXADDR | x);
	}

	if (y > 0x1F && y < 0x28 ) {
		LCD_Write(LCD_COMMAND, setYAddressBank5);
		LCD_Write(LCD_COMMAND, LCD_SETXADDR | x);
	}

	if (y > 0x27 && y < 0x30 ) {
		LCD_Write(LCD_COMMAND, setYAddressBank6);
		LCD_Write(LCD_COMMAND, LCD_SETXADDR | x);
	}
}
...
unsigned char LCD_DrawPixel(unsigned char x, unsigned char y, LCD_Pixel_t pixel)
{
	...
	return LCD_Buffer[x + (y / 8) * LCD_WIDTH];
}
...
{
	unsigned char c_height, c_width, i, b, j ,result;
	...
	for (i = 0; i < c_width; i++) {
		...
		for (j = 0; j < c_height; j++ ){
			if (color == LCD_Pixel_Set) {
				result = LCD_DrawPixel(LCD_x, (LCD_y+j),
			 ...
			 unsigned char y1, LCD_Pixel_t color)
{
	...
	if (dx == 0) {
		do {
			printk(KERN_WARNING "Value of x0: %d, value of y0: %d", x0, y0);
			LCD_GotoXY(x0, y0);
			result = 0x00;
			result = LCD_DrawPixel(x0, y0, color);
			LCD_Write(LCD_DATA, result);
		//	LCD_Refresh();
			y0++;
		} while (y1 >= y0);
	...
	if (dx > dy) {
		...
		result = 0x00;
		result = LCD_DrawPixel(x0, y0, color);
		printk(KERN_WARNING "Result: %d", result);
		LCD_Write(LCD_DATA, result);
	} else {
		temp = 2 * dx - dy;
		while (y0 != y1) {
