...
//static ssize_t dev_read(struct file *filep, const char __user *buf, size_t len,			 	loff_t *offset);
//{
//	int ret;
                        ...
                        loff_t *offset)
{
	...
	printk(KERN_WARNING "Value of string: %s, value of pixel: %d, value of font: %d", str->message, str->pixel, str->font);
...
static long dev_ioctl(struct file *filep, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_DRAW_LINE:
			...
			printk(KERN_WARNING "x0: %d, y0: %d,x1 : %d, y1: %d", shape->x0, shape->y0
					, shape->x1, shape->y1);
